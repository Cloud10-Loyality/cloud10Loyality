...
natsClient.connect("cloud10LMS", CLIENT_ID, "http://nats-srv:4222").then(async (
  ...
  await mongoose.connect(DB!).then(conn => {
    console.log(
      `[User-Service DB]: Database successfully running on ${conn.connection}`
    );
    app.listen(PORT, () => {
      console.log(
  ...
  console.log(`${err}: Error connecting to NATS`);
})
